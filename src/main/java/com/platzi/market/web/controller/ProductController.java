...
public class ProductController {
    ...
    public ResponseEntity<Product> getProduct(
            @ApiParam(value = "The id of the product", required = true, example = "8")
    ...
    @ApiOperation("Search all supermarket product with a specific category")
    @ApiResponses({
            @ApiResponse(code = 200, message = "OK" ),
    ...
    @ApiOperation("Delete a supermarket product")
    @ApiResponses({
            @ApiResponse(code = 200, message = "OK" ),
            @ApiResponse(code = 404, message = "Product not found")
    })
